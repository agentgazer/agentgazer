...
# Usage: curl -fsSL https://agentgazer.org/install.sh | sh
#
# This script:
...
#   AGENTTRACE_HOME   Install directory (default: ~/.agentgazer)
...
WRAPPER_PATH="$AGENTTRACE_BIN/agentgazer"
...
if [ -x "$AGENTTRACE_HOME/node/bin/node" ]; then
  ...
  echo "Error: Node.js not found. Re-run the AgentGazer install script." >&2
  exit 1
fi
exec "$NODE" "$AGENTTRACE_HOME/lib/lib/node_modules/@agentgazer/cli/dist/cli.js" "$@"
'
