#!/bin/sh
# AgentTrace install script
# Usage: curl -fsSL https://agenttrace.org/install.sh | sh
#
# This script:
#   1. Detects your platform (macOS/Linux, x64/arm64)
#   2. Downloads Node.js if not present or < 18
#   3. Installs agenttrace via npm into ~/.agenttrace/lib/
#   4. Creates a wrapper script at /usr/local/bin/agenttrace
#
# Environment variables:
#   AGENTTRACE_HOME   Install directory (default: ~/.agenttrace)
#   AGENTTRACE_BIN    Wrapper location (default: /usr/local/bin)
...
AGENTTRACE_BIN="${AGENTTRACE_BIN:-/usr/local/bin}"
NODE_DIR="$AGENTTRACE_HOME/node"
LIB_DIR="$AGENTTRACE_HOME/lib"
WRAPPER_PATH="$AGENTTRACE_BIN/agenttrace"
...
detect_platform() {
  ...
  case "$OS" in
    ...
    *)
      ...
      echo "    npm install -g @agenttrace/cli"
      exit 1
      ;;
  esac
...
# --- Install agenttrace via npm -----------------------------------------------

install_agenttrace() {
  info "Installing agenttrace..."
  ...
  if [ ! -f "$LIB_DIR/lib/node_modules/@agenttrace/cli/dist/cli.js" ]; then
    error "Installation failed — cli.js not found"
    exit 1
  fi

  success "agenttrace installed to $LIB_DIR"
}
...
# AgentTrace wrapper — auto-generated by install.sh
AGENTTRACE_HOME="${AGENTTRACE_HOME:-'"$HOME"'/.agenttrace}"
if [ -x "$AGENTTRACE_HOME/node/bin/node" ]; then
  ...
  echo "Error: Node.js not found. Re-run the AgentTrace install script." >&2
  exit 1
fi
exec "$NODE" "$AGENTTRACE_HOME/lib/lib/node_modules/@agenttrace/cli/dist/cli.js" "$@"
'

  # Try without sudo first
  if mkdir -p "$AGENTTRACE_BIN" 2>/dev/null && \
    ...
    info "Need elevated permissions to write to $AGENTTRACE_BIN"
    sudo sh -c "mkdir -p '$AGENTTRACE_BIN' && printf '%s' '$WRAPPER_CONTENT' > '$WRAPPER_PATH' && chmod +x '$WRAPPER_PATH'"
...
main() {
  ...
  # Get version
  AT_VERSION="$("$RESOLVED_NODE" "$LIB_DIR/lib/node_modules/@agenttrace/cli/dist/cli.js" version 2>/dev/null || echo "unknown")"

  echo ""
  echo "  ─────────────────────────────────"
  success "AgentTrace $AT_VERSION installed!"
  echo ""
  echo "  Get started:"
  echo "    agenttrace              Launch AgentTrace (server + proxy + dashboard)"
  echo ""
  echo "  Useful commands:"
  echo "    agenttrace onboard      First-time setup"
  echo "    agenttrace doctor       Check system health"
  echo "    agenttrace --help       Show all commands"
  echo ""
  echo "  To uninstall:"
  echo "    agenttrace uninstall"
  echo ""
}
