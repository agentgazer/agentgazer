...
當作業系統鑰匙圈可用時（macOS 鑰匙圈、Linux libsecret），金鑰會透過鑰匙圈存放。
在 SSH、無頭環境或 Docker 中，金鑰以 AES-256-GCM 加密，使用機器衍生金鑰。
金鑰永遠不會以明文存放。

### 同一台機器上的其他程序能讀取我的 API 金鑰嗎？

如果作業系統鑰匙圈可用，金鑰受作業系統保護。在無頭模式下，加密檔案可防止隨意讀取，
但以相同使用者身分執行的程序理論上可以推導出相同的加密金鑰。為了最大程度的保護，
建議使用有作業系統鑰匙圈支援的桌面環境。

### 金鑰在啟動後是否保留在記憶體中？

是的。金鑰在 `agenttrace start` 啟動時解密一次，在代理的存活期間保存於記憶體中。運行期間不會將金鑰寫回磁碟。如果你用 `agenttrace providers set` 更新了金鑰，需要重新啟動 `agenttrace start` 才會生效。
