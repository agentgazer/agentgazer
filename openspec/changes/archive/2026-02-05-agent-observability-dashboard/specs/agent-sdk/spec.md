...
WHEN a developer calls `AgentGazer.init({ apiKey: "ak_test123", agentId: "bot-1", endpoint: "http://localhost:4000" })`
...
WHEN a developer calls `AgentGazer.init({ agentId: "bot-1" })` without providing `apiKey`
THEN the function MUST throw an error indicating that `apiKey` is required.

#### Scenario: Initialization with missing agentId

WHEN a developer calls `AgentGazer.init({ apiKey: "ak_test123" })` without providing `agentId`
...
The SDK SHALL buffer events internally and flush them in batches to the ingest API. The SDK MUST flush the buffer when either of the following conditions is met: 5 seconds have elapsed since the last flush, or the buffer contains 50 or more events. Whichever threshold is reached first MUST trigger the flush.

#### Scenario: Flush on time threshold

WHEN a developer enqueues 3 events and 5 seconds elapse without reaching 50 events
THEN the SDK MUST flush the buffer and send all 3 events to the ingest API in a single batch request.

#### Scenario: Flush on buffer size threshold

WHEN a developer enqueues 50 events in under 5 seconds
THEN the SDK MUST immediately flush the buffer and send all 50 events to the ingest API in a single batch request.

#### Scenario: Combined threshold behavior

WHEN a developer enqueues 30 events in the first 2 seconds and then 20 more events 1 second later (totaling 50)
THEN the SDK MUST flush at the moment the 50th event is enqueued, without waiting for the 5-second timer.
...
WHEN a TypeScript developer imports `{ AgentGazerConfig }` from the SDK package
...
The SDK SHALL use a default endpoint URL pointing to the cloud-hosted ingest API. The endpoint MUST be overridable via the `endpoint` property in the `AgentGazer.init()` configuration. When `endpoint` is provided, all event flushes MUST be sent to the specified URL instead of the default.

#### Scenario: Default endpoint used when not specified

WHEN a developer calls `AgentGazer.init({ apiKey: "ak_test123", agentId: "bot-1" })` without specifying `endpoint`
THEN the SDK MUST send events to the default cloud API endpoint.

#### Scenario: Custom endpoint override

WHEN a developer calls `AgentGazer.init({ apiKey: "ak_test123", agentId: "bot-1", endpoint: "https://custom.example.com/api/events" })`
