...
The CLI SHALL generate a cryptographically random token (32 bytes, hex-encoded) on first run and store it in `~/.agentwatch/config.json` as `{ "token": "<hex>" }`. If the config file already exists with a valid token, the existing token SHALL be reused.

#### Scenario: First run generates token
- **WHEN** the CLI starts and `~/.agentwatch/config.json` does not exist
- **THEN** a new random token is generated, saved to config, and printed to the terminal

#### Scenario: Subsequent run reuses token
- **WHEN** the CLI starts and `~/.agentwatch/config.json` exists with a valid token
- **THEN** the existing token is used and printed to the terminal

### Requirement: Token reset command
The CLI SHALL support a `--reset-token` flag that generates a new token, replacing the existing one in `~/.agentwatch/config.json`.
