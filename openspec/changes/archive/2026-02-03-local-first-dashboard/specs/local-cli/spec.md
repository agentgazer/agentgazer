...
The package SHALL export a bin command `agentgazer` that starts the local API server, LLM proxy, and serves the dashboard — all in a single Node.js process. The command SHALL be runnable via `npx agentgazer`.
...
- **WHEN** the user runs `npx agentgazer --port 9090 --proxy-port 5000`
...
The CLI SHALL ensure `~/.agentgazer/` exists on startup. If the directory does not exist, it SHALL be created. The config file (`config.json`) and database (`data.db`) SHALL reside in this directory.

#### Scenario: First run creates directory
- **WHEN** the CLI starts and `~/.agentgazer/` does not exist
...
- **WHEN** the user runs `npx agentgazer` without `--no-open`
- **THEN** the default browser opens `http://localhost:8080`

#### Scenario: Browser opening suppressed
- **WHEN** the user runs `npx agentgazer --no-open`
...
- **WHEN** the user runs `npx agentgazer --help`
...
- **WHEN** the user runs `npx agentgazer --reset-token`
