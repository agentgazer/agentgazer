...
- [ ] 1.1 Generate random salt on first use and persist to `~/.agentgazer/config.json`
- [ ] 1.2 Add migration: detect old fixed-salt encrypted secrets and re-encrypt with new salt
- [ ] 1.3 Update `secret-store.ts` to read salt from config
...
- [ ] 2.1 Update `DEFAULT_SECURITY_CONFIG` to enable `api_key` and `credit_card` masking by default
- [ ] 2.2 Ensure existing user configs are not overwritten (only applies to new installations)
- [ ] 2.3 Add test: new agent gets default masking enabled
- [ ] 2.4 Update docs to reflect new defaults

## 3. Webhook HMAC Signing

- [ ] 3.1 Add `webhook_secret` field to alert rule configuration
- [ ] 3.2 Implement HMAC-SHA256 signing for webhook payloads
- [ ] 3.3 Include `X-AgentGazer-Signature` header in webhook requests
