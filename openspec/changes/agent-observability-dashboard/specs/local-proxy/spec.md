...
The proxy SHALL be startable via the command `npx agentwatch-proxy --api-key <key> --agent-id <id>`. The `--api-key` and `--agent-id` flags MUST be required. If either is missing, the CLI MUST print a usage error and exit with a non-zero status code.

#### Scenario: Start proxy with required flags

WHEN a user runs `npx agentwatch-proxy --api-key ak_test123 --agent-id bot-1`
THEN the proxy MUST start and begin listening for requests on the configured port.

#### Scenario: Missing api-key flag

WHEN a user runs `npx agentwatch-proxy --agent-id bot-1` without `--api-key`
...
WHEN a user runs `npx agentwatch-proxy --api-key ak_test123` without `--agent-id`
