...
**Mitigation:** The proxy is strictly optional. The SDK alone provides agent health monitoring (heartbeats), error tracking, and manual event reporting. The proxy adds automated LLM call tracking, but agents work on the dashboard without it. For users who do adopt the proxy, the setup is a single command (`npx @agentwatch/proxy start`) and one environment variable change. The proxy also provides a CLI health check (`agentwatch-proxy status`) so users can verify it is running.
