...
describe("parseProviderResponse", () => {
  ...
  describe("Cohere format", () => {
    it("parses a successful Cohere response", () => {
      const body = {
        meta: {
          billed_units: {
            input_tokens: 500,
      ...
      const result = parseProviderResponse("cohere", body, 200);
      expect(result).toEqual({
        model: null,
        tokensIn: 500,
        tokensOut: 250,
    ...
    it("always returns null for model (Cohere does not echo model)", () => {
      const body = {
        meta: {
          billed_units: {
            input_tokens: 100,
    ...
    it("returns null tokensTotal when only input_tokens present", () => {
      const body = {
        meta: {
          billed_units: {
            input_tokens: 100,
    ...
    it("parses a Cohere error response (top-level message)", () => {
      const body = { message: "Unauthorized" };
      const result = parseProviderResponse("cohere", body, 401);
      expect(result).toEqual({
        model: null,
        tokensIn: null,
        tokensOut: null,
    ...
    it("handles Cohere error with no message (falls back to HTTP status)", () => {
      const result = parseProviderResponse("cohere", {}, 500);
      expect(result!.statusCode).toBe(500);
      expect(result!.errorMessage).toBe("HTTP 500");
    });

    it("handles Cohere response with empty meta", () => {
      const body = { meta: {} };
      const result = parseProviderResponse("cohere", body, 200);
      expect(result!.tokensIn).toBeNull();
      expect(result!.tokensOut).toBeNull();
