...
describe("RateLimiter", () => {
  ...
  it("allows requests when no config is set for a provider", () => {
  ...
  it("allows requests within the configured limit", () => {
    const limiter = new RateLimiter({
      openai: { maxRequests: 3, windowSeconds: 60 },
    });
  ...
  it("tracks providers independently", () => {
    const limiter = new RateLimiter({
      ...
      anthropic: { maxRequests: 1, windowSeconds: 60 },
    });
  ...
  it("allows requests after the window expires", () => {
    ...
    const limiter = new RateLimiter({
      openai: { maxRequests: 1, windowSeconds: 10 },
    });
  ...
  it("returns appropriate retry-after value", () => {
    ...
    const limiter = new RateLimiter({
      openai: { maxRequests: 1, windowSeconds: 30 },
    });

    limiter.check("openai"); // consume the slot
  ...
  it("allows unconfigured providers even when others are limited", () => {
    const limiter = new RateLimiter({
      openai: { maxRequests: 1, windowSeconds: 60 },
    });
    ...
    // google has no config — should always be allowed
    expect(limiter.check("google").allowed).toBe(true);
    expect(limiter.check("google").allowed).toBe(true);
  });

  it("hasConfig returns correct values", () => {
    const limiter = new RateLimiter({
      openai: { maxRequests: 10, windowSeconds: 60 },
    });

    expect(limiter.hasConfig("openai")).toBe(true);
    expect(limiter.hasConfig("anthropic")).toBe(false);
  });
  ...
  it("handles sliding window correctly with staggered requests", () => {
    ...
    const limiter = new RateLimiter({
      openai: { maxRequests: 3, windowSeconds: 10 },
    });
