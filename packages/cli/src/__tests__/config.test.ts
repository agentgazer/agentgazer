...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "agentgazer-config-test-"),
  );
...
describe("config", () => {
  ...
  describe("getConfigDir()", () => {
    it("returns a path under the home directory", async () => {
      ...
      expect(getConfigDir()).toBe(path.join(tmpDir, ".agentgazer"));
    });
  });
  ...
  describe("getDbPath()", () => {
    it("returns the database file path under the config directory", async () => {
      ...
      expect(getDbPath()).toBe(
        path.join(tmpDir, ".agentgazer", "data.db"),
      );
    });
  });
  ...
  describe("readConfig()", () => {
    ...
    it("returns the config when a valid config file exists", async () => {
      const configDir = path.join(tmpDir, ".agentgazer");
  ...
  describe("ensureConfig()", () => {
    ...
    it("persists the config to disk as valid JSON", async () => {
      ...
      const configFile = path.join(tmpDir, ".agentgazer", "config.json");
